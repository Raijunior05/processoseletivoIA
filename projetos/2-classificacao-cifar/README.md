...
A execução do artefato de Edge (model.tflite) isolado do ambiente de treinamento provou-se altamente estável, cravando uma precisão local de 100% no batch de teste acima. É notório observar que o decaimento brutal na precisão numérica dos pesos (de 32-bits para 8-bits imposto pela quantização) não corrompeu a capacidade discriminativa das feature maps da CNN. O modelo demonstrou alta confiabilidade separando domínios semânticos distintos, variando de estruturas rígidas e linhas retas (airplane, ship) a classes orgânicas mais difusas e com fundos ruidosos (cat, frog).